...
1. **Install Dependencies**  
   Ensure you have installed the required dependencies for the AIRO project. This includes the setup.py file to ensure all modules can be imported. 
   
   ```pip install -e .```
 
2. **Run Training**  
   from the AIRO directory, use the provided training scripts to train your GDQN agent. Example:
   ```bash
   python RL/GDQN/agent.py 
   ```

## Directory Structure
- `data_as_graph.py`: converts data into a graph 
- `envs/`: Contains the custom OpenAI Gymnasium environment.
- `agent.py`: Script to train GDQN models.
- `utils/`: Helper functions and utilities specific to GDQN.
